def calculate_similarity(input_company, target_companies, environmental_data, risk_data):
    # Calculate cosine similarity between input company and target companies based on environmental impact and risk
    input_environmental_impact = environmental_data.get(input_company,
                                                        0)  # Assume 0 if environmental data not available
    input_risk = risk_data.get(input_company, 3)  # Assume risk of 3 if data not available
    ...
    for company in target_companies:
        ...
        similarity_score = (input_environmental_impact * target_environmental_impact) / (
                    (input_risk + 1) * (target_risk + 1))
        similarities[company] = similarity_score

    return similarities


def recommend_similar_company(input_company, input_sector, sector_list, environmental_data, risk_rating):

    # Calculate similarity based on environmental impact
    similarities = calculate_similarity(input_company, sector_list, environmental_data, risk_rating)
...
# Example usage
input_company = 'AAPL'
input_sector = 'information_technology'
input_risk_rating = 3
...
all_recommendations, recommended_company = recommend_similar_company(input_company, input_sector, companies_in_sector, company_to_environmental_impact, company_to_risk_rating)
top_three_recommendations = dict(sorted(all_recommendations.items(), key=lambda item: item[1], reverse=True)[:3])
for company, recommendation in top_three_recommendations.items():
    print(company)